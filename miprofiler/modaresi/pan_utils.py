...
TRUTH_FILE_COLUMNS = ['id', 'gender', 'age',
                      'extroverted', 'stable', 'agreeable',
                      'conscientious', 'open']


def parse_xml(filename, parser='xml', clean_text=True):
    """parses xml files, that contains the author and its documents"""
    base = os.path.basename(filename)
    idx = str(base[:-4])
    posts = []
    logger.debug('Parse %s', filename)
        ...
        for d in author.find_all('document'):
            ...
            if clean_text:
                try:
                    ...
                    text = (post_soup.get_text()).strip()
                except UserWarning:
                    pass
            if not text:
                logger.debug('No text in document: %s', d)
            post = dict(text=text)
            post.update(d.attrs)
            post['markup'] = str(d)
            posts.append(post)
